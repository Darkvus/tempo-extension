<template>
  <div class="p-4 space-y-4">
    ...
    <button
      @click="openWebApp"
      class="flex items-center justify-center gap-1.5 text-xs text-neutral-500 hover:text-neutral-300 transition-colors w-full"
    >
...
async function openWebApp() {
  ...
  if (company) {
    params.set('company', JSON.stringify(company))
    params.set('mode', 'professional')
  }
  browser.tabs.create({ url: `https://tempo.darkvus.com?${params.toString()}` })
}
